...
Ce tutoriel documente le processus de déploiement d'une application web interactive sur un cluster Kubernetes. La configuration présentée est spécifiquement adaptée au [SSP Cloud](https://datalab.sspcloud.fr/home). L'objectif du procesus est d'automatiser le déploiement au maximum pour favoriser sa reproductibilité : le processus prend en entrée les données nécessaires à l'application, et renvoie en output une URL qui permet d'accéder à l'application sur internet.

La procédure sera illustrée à travers l'exemple du déploiement d'une [application R Shiny](https://shiny.rstudio.com/). On utilisera deux dépôts GitHub qui font office de *template* :
